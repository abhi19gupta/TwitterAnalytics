...
dag = DAG(dag_id='third', default_args=args,schedule_interval=None)
...
def execute_query(node_name,**context):
	...
	for x in input_vars:
		if(mapping[node_name][x]=="-"):
			...
			inp = context['task_instance'].xcom_pull(task_ids=get_task_from_node(mapp[0]),dag_id = "third",key=mapp[1])
			print(inp)
			inputs[x] = inp
	print("========================================")
