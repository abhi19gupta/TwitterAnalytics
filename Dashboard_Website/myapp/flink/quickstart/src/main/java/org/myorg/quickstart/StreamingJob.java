...
public class StreamingJob {
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length != 1){
			System.err.println("USAGE:\nSocketTextStreamWordCount <jobname>");
			return;
		}
				...
				.filter(new FilterFunction<JsonNode>() {
					...
					public boolean filter(JsonNode jsonNode) {
						try {
							...
							for (final JsonNode hashtagNode : jsonNode.get("entities").get("hashtags"))
								hashtags.add(hashtagNode.get("text").asText());
							boolean isSatisfied = hashtags.contains("1") || hashtags.contains("2");
							return isSatisfied;
						} catch (Exception e) {
							return false;
						}
				...
				.flatMap(new FlatMapFunction<JsonNode, JsonNode>() {
					...
					public void flatMap(JsonNode jsonNode, Collector<JsonNode> out) {
						try {
							...
							for (final JsonNode urlNode : jsonNode.get("entities").get("urls"))
								urls.add(urlNode.get("expanded_url").asText());
							if (urls.isEmpty())
