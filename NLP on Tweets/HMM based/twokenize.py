...
def regex_or(*items):
    return '(?:' + '|'.join([x.decode("utf-8") for x in items if type(x)=="bytes"]) + ')'
...
print(type(bfLeft),type(bfCenter),type(bfRight))
basicface = "(?:" +bfLeft.decode("utf-8")+bfCenter+bfRight+ ")|" +s3+ "|" +s4+ "|" + s5

eeLeft = r"[＼\\ƪԄ\(（<>;ヽ\-=~\*]+"
eeRight= u"[\\-=\\);'\u0022<>ʃ）/／ノﾉ丿╯σっµ~\\*]+".encode('utf-8')
eeSymbol = r"[^A-Za-z0-9\s\(\)\*:=-]"
eastEmote = eeLeft + "(?:"+basicface+"|" +eeSymbol+")+" + eeRight.decode("utf-8")
